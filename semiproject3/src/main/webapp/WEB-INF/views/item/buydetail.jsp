	...
	<form action="/orders/insert" method="post">
		...
		<div class="row">
			<table class="table">
				<tbody>
					<tr>
						<th class="w-50" style="height: 480px;" rowspan="4"><c:forEach
								var="buylistView" items="${buyImageList}">
								<c:if test="${buylistView.imageMain == 1}">
									<img src="/image/download/${buylistView.imageNo}" width="200">
								</c:if>
							</c:forEach> <c:forEach var="buylistView" items="${buyImageList}">
								<c:if test="${buylistView.imageMain == 0}">
									<img src="/image/download/${buylistView.imageNo}" width="100">
								</c:if>
							</c:forEach></th>
						<th colspan="2"><input type="hidden" name="customerId"
							value="${loginId}"> <input type="hidden" name="itemNo"
							value="${itemDto.itemNo}"> <input type="text"
							name="itemName" value="${itemDto.itemName}" readonly
							class="input input-none"></th>
					</tr>
					<tr>
						<th>Price</th>
						<td><fmt:formatNumber value="${itemDto.itemPrice}"
								pattern="#,##0원"></fmt:formatNumber> <input type="hidden"
							name="itemPrice" value="${itemDto.itemPrice}"></td>
					<tr>
						<th>Option</th>
						<td><select class="input w-100" id="itemColor"
							name="itemColor">
								...
								<c:forEach var="itemDto" items="${buylist}">
									<option value="${itemDto.itemColor}"
										...
										data-cnt="${itemDto.itemTotalCnt}">
										${itemDto.itemColor}/${itemDto.itemSize}(잔여수량:${itemDto.itemTotalCnt})
									</option>
								</c:forEach>
						</select> <input class="input w-100" type="hidden" name="itemSize" value="">
						</td>
					</tr>
					<tr>
						<th>Qnty</th>
						<td>
							...
							<input class="input w-100" type="number" id="itemCnt"
							name="itemCnt" min="0" max=""> <!--                <button class="plus-btn" type="button">+</button> -->
						</td>
					</tr>
					<tr>
						<td class="right">${itemDto.itemLikeCnt} <c:if
								test="${isLike == null}">
                  ♥
               </c:if> <c:if test="${isLike == true}">
								<a href="like?itemNo=${itemDto.itemNo}">♥</a>
							</c:if> <c:if test="${isLike == false}">
								<a href="like?itemNo=${itemDto.itemNo}">♡</a>
							</c:if>
						</td>
						<td colspan="2" align="right"><a
							href="/review/insert?itemNo=${itemDto.itemNo}">리뷰달기</a>
							<button type="button" class="btn btn-positive">구매하기</button>
							<button type="button" class="btn btn-positive"
								onclick="goCart(${itemDto.itemNo})">장바구니</button> <a
							href="buylist">목록으로</a></td>
					</tr>
					<tr>
					...
					<table class="table">
						...
						<tbody align="center">
							<c:forEach var="list" items="${reviewList}">
								<tr>
									<td><c:if test="${list.reviewStar==1}">★</c:if> <c:if
											test="${list.reviewStar==2}">★★</c:if> <c:if
											test="${list.reviewStar==3}">★★★</c:if> <c:if
											test="${list.reviewStar==4}">★★★★</c:if> <c:if
											test="${list.reviewStar==5}">★★★★★</c:if> <c:set var="total"
											value="${total+list.reviewStar}" /></td>
									<td>${list.reviewPackaging}</td>
									<td>${list.reviewShipping}</td>
									<td><c:out
											value="${fn:substring(list.customerId, 0, fn:length(list.customerId) - 4)}" />
										****</td>
...
<<<<<<< HEAD
									...
									<td><img src="/reviewImage/download/${list.imageNo}"
										width="100"></td>
									...
									<td><c:choose>
											<c:when test="${loginId != list.customerId}">
												<a
													href="/review/report?reviewNo=${list.reviewNo}&itemNo=${itemDto.itemNo}">신고</a>
											</c:when>
											<c:otherwise>
									...
									<c:choose>
										<c:when test="${list.reviewBlind}">
											<td><a
												href="/review/blind?reviewNo=${list.reviewNo}&itemNo=${itemDto.itemNo}">블라인드<br>해제
											</a></td>
										</c:when>
										<c:otherwise>
											<td><a
												href="/review/blind?reviewNo=${list.reviewNo}&itemNo=${itemDto.itemNo}">블라인드<br>설정
											</a></td>
										</c:otherwise>
									</c:choose>
