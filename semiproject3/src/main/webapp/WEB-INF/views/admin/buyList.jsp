...
<li>
	<c:choose>
		<c:when test="${not vo.isFirst()}">
			<a href="admin-buylist?p=${vo.firstBlock()}&${vo.parameter()}">
...
<li>
	<c:choose>
		<c:when test="${vo.hasPrev()}">
			<a href="admin-buylist?p=${vo.prevBlock()}&${vo.parameter()}">
...
<c:forEach var="i" begin="${vo.startBlock()}" end="${vo.endBlock()}" step="1">
	<li <c:if test="${i==param.p}">class="on"</c:if>><a href="admin-buylist?p=${i}&${vo.parameter()}">${i}</a></li>
</c:forEach>
...
<li>
	<c:choose>
		<c:when test="${vo.hasNext()}">
			<a href="admin-buylist?p=${vo.nextBlock()}&${vo.parameter()}">
...
<li>
	<c:choose>
		<c:when test="${not vo.isLast()}">
			<a href="admin-buylist?p=${vo.lastBlock()}&${vo.parameter()}">
